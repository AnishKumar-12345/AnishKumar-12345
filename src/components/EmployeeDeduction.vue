...
<v-col cols="12" md="3">
            <v-select
              :items=" this.Deductiondata"
              label="Select Deduction Name"
              v-model="EmployeeAllowanceModel.deductionName"
              dense
              multiple
              outlined
            ></v-select>
...
<v-col cols="12 text-right" md="12">
...
export default {
  ...
  methods: {
    ...
    PostBranch() {
      ...
      this.EmployeeDeductionsave(this.selected, reqJSON).then(response => {
    ...
    getList() {
      ...
      this.getDeduction().then(response => {
        ...
        if (response.status == "200") {
          console.log("resdata", response.data.Deductions);
          //  this.allBranchNames=response.data
          for (var i = 0; i < response.data.Deductions.length; i++) {
            this.Deductiondata.push(response.data.Deductions[i].deductionName);
          }

          console.log("Allowancesdata", this.Deductiondata);
        }
      });
    },
