<template>
  <div>
      ...
      <v-img class="mx-2" max-height="40" max-width="40" contain></v-img>
        <router-link to="/UserDashboard" style="text-decoration: none; color: inherit;margin-left:100px">
...
<!-- <v-icon large  color="white" style="padding:10px;" @click="logout()">mdi-logout </v-icon> -->
    ...
    <v-card width="90%" style="margin-left:110px;margin-top:20px" >
     ...
     <v-toolbar color="#1565c0" >
        <!-- <v-row style="margin-top:10px;margin-left:30px">
          ...
          <v-col>
            <v-btn @click="getallApis(selected)" style="background: red;">Retrieve</v-btn>
          </v-col>
        </v-row> -->
 ...
 <div style="margin-top:20px;margin-bottom:5px; text-align:right;">
   <v-btn v-show="showText==false" @click="showText=true">Add</v-btn>
  </div >
     ...
     sendItem(item){
      ...
      editedDesig(){
        ...
        if (response.status == 200) {
          ...
          this.snackbar = true;
          setTimeout(() => {  this.$router.push({name:"UserDashboard" }) }, 2000);
